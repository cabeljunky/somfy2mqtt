...
class Event:
    ...
    def __init__(self, active, repeatType, repeatValue, timeType, timeValue, shutterAction, shutterIds):
        ...
        if (repeatValue == 'weekday') and not all(
                elem in ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] for elem in repeatValue):
        ...
        if (timeType == "astro") and not ((astro_parts[0] in ('sunset', 'sunrise')) and (
                (len(astro_parts) == 1) or (astro_parts[1] == None or int(astro_parts[1])))):
...
class Schedule(MyLog):
    ...
    def addOneEventByTime(self, shutterIds, shutterAction, hour, minute):
        try:
            evt = Event('active', 'once', datetime.datetime.today().strftime('%Y/%m/%d'), "clock",
                        str(hour) + ":" + str(minute), shutterAction, shutterIds)
    ...
    def addRepeatEventByTime(self, shutterIds, shutterAction, hour, minute, weekdays):
        try:
            evt = Event('active', 'weekday', weekdays, "clock", str(hour) + ":" + str(minute), shutterAction,
                        shutterIds)
            self.addEvent(self.getNewId(), evt)
        except ValueError as ex:
            self.LogError("Failed to add event: " + str(ex))
    ...
    def loadScheudleFromConfig(self):
        ...
        for id, data in self.config.Schedule.items():
            ...
            evt = Event(data['active'], data['repeatType'], repeatValue, data['timeType'], data['timeValue'],
                        data['shutterAction'], data['shutterIds'].split("|"))
    ...
    def addSchedule(self, data):
        ...
        repeatValueStr = data['repeatValue'][0] if (data['repeatType'][0] == "once") else "|".join(
            data['repeatValue[]'])
        ...
        self.config.WriteValue(str(id),
                               active + "," + repeatType + "," + repeatValueStr + "," + timeType + "," + timeValue + "," + shutterAction + "," + shutterIdsStr,
                               section="Scheduler");
    ...
    def getScheduleAsDict(self):
        ...
        for id, evt in self.schedule.items():
            if evt.active != "deleted":
                item = {'active': evt.active, 'repeatType': evt.repeatType, 'repeatValue': evt.repeatValue,
                        'timeType': evt.timeType, 'timeValue': evt.timeValue, 'shutterIds': evt.shutterIds,
                        'shutterAction': evt.shutterAction}
                obj[id] = item
        return obj
...
class Scheduler(threading.Thread, MyLog):
    ...
    def updateSchedule(self):
        ...
        self.LogInfo("Today is " + date + ", a " + weekday + ", Sunrise is at " + str(
            sunrise.time()) + " and Sunset is at " + str(sunset.time()));

        self.currentSchedule = {}
        for id, event in self.schedule.getSchedule().items():
            if ((event.active == "active") and (
                    ((event.repeatType == 'weekday') and (weekday in event.repeatValue)) or (
                    (event.repeatType == 'once') and (date == event.repeatValue)))):
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
            ...
            if ((self.lastScheduleUpdateTime < currentScheduleUpdateTime) or (
                    self.weekday != datetime.datetime.today().weekday())):
                self.updateSchedule()
                self.weekday = datetime.datetime.today().weekday()
                ...
                if (eventTimeStr <= timeNowStr):
                    for eventDetail in eventDetails:
                        for shutterId in eventDetail[0]:
                            try:
                                self.LogInfo("Send action \"" + eventDetail[
                                    1] + "\" to shutterId \"" + shutterId + "\" at " + datetime.datetime.now().strftime(
                                    "%Y/%m/%d %H:%M:%S"))
                                if (eventDetail[1].startswith("up")):
                                    s = eventDetail[1][2:].strip()
                                    s1 = int(s) if s else -1
                                    if (0 < s1 < 100):
                                        if (self.shutter.getPosition(
                                                shutterId) < s1):  # Is Shutter below requested Position?
                                            self.shutter.risePartial(shutterId, s1)
                                        else:
                                            self.LogWarn("Send action \"" + eventDetail[
                                                1] + "\" to shutterId \"" + shutterId + "\" was canceled! Shutter was already at same or above requested position")
                                    else:
                                    ...
                                    if (0 < s1 < 100):
                                        if (self.shutter.getPosition(
                                                shutterId) > s1):  # Is Shutter above requested Position?
                                            self.shutter.lowerPartial(shutterId, s1)
                                        else:
                                            self.LogWarn("Send action \"" + eventDetail[
                                                1] + "\" to shutterId \"" + shutterId + "\" was canceled! Shutter was already at same or below requested position")
                                    else:
                                        for i in range(self.config.SendRepeat):
